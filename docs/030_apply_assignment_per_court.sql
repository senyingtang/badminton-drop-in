-- 每面場獨立輪次：apply_assignment 改為 (session, court_no, round_no, payload)
-- 僅建立該面場一筆 draft round + 一場 match（payload 內可只含該場，仍會過濾 courtNo）。
-- 須先執行 029_rounds_per_court_migration.sql。

drop function if exists public.apply_assignment_recommendation_and_create_round(uuid, integer, jsonb);
...
begin
  ...
  loop
    ...
    if v_court_no <> input_court_no then
      continue;
    end if;
  ...
  insert into public.rounds (
    ...
    input_session_id, input_court_no, input_round_no, 'draft', v_rec_id
  )
