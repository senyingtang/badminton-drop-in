...
--
-- 注意：函式使用 LANGUAGE sql（不用 PL/pgSQL 區域變數），避免 Supabase 編輯器
-- 將 v_host 等誤判為「新資料表」或執行時出現 relation "v_host" does not exist。
...
as $$
  select
    coalesce(pgs.rented_courts_display_mode, 'below')::text as rented_courts_display_mode,
    coalesce(pgs.theme_preset, 'indigo')::text as theme_preset,
    coalesce(pgs.theme_custom, '{}'::jsonb) as theme_custom
  from public.sessions s
  left join public.pickup_group_settings pgs on pgs.host_user_id = s.host_user_id
