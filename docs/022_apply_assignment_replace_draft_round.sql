-- Before creating a new draft round, remove any existing DRAFT round for the same
-- (session_id, round_no) so "確認並建立" can succeed after preview/regenerate retries.
-- (Unique constraint: rounds_session_id_round_no_key)
